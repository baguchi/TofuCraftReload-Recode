...
public class HellTofuBlock extends Block {
	public static final IntegerProperty AGE = BlockStateProperties.AGE_7;

	public HellTofuBlock(Properties properties) {
		super(properties);
	}
	...
	public void animateTick(BlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		...
		if (isUnderWeight(worldIn, pos) &&
			...
			worldIn.addParticle(ParticleTypes.CRIMSON_SPORE, d0, d1, d2, 0.0D, 0.0D, 0.0D);
		}
	}

	public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
		super.randomTick(state, worldIn, pos, random);
		if (isUnderWeight(worldIn, pos) && worldIn.getBlockState(pos.below(2)).is(BlockTags.FIRE)) {
