 ...
 */
public class TofuStructureUpdater implements DataProvider {
	private final String basePath;
	private final PackOutput output;
	private final MultiPackResourceManager resources;

	public TofuStructureUpdater(String basePath, PackOutput output, ExistingFileHelper helper) {
	...
	public CompletableFuture<?> run(@Nonnull CachedOutput cache) {
		try {
			for (var entry : this.resources.listResources(this.basePath, $ -> true).entrySet())
				if (entry.getKey().getNamespace().equals(TofuCraftReload.MODID))
