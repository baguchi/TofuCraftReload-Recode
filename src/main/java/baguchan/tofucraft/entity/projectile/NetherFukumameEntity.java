...
public class NetherFukumameEntity extends ThrowableEntity {
	...
	public void func_70103_a(byte id) {
		if (id == 3) {
			double d0 = 0.08D;
			for (int i = 0; i < 6; i++)
				this.level.func_195594_a((IParticleData) new ItemParticleData(ParticleTypes.field_197591_B, new ItemStack(TofuItems.SEEDS_SOYBEANS_NETHER)), getX(), getY(), getZ(), (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D);
		} else if (id == 4) {
			this.level.func_195594_a((IParticleData) ParticleTypes.field_239814_an_, getX(), getY(), getZ(), (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D);
		}
	}
	...
	protected void func_213868_a(EntityRayTraceResult p_213868_1_) {
		...
		if (!p_213868_1_.func_216348_a().func_230279_az_() && func_70027_ad())
			p_213868_1_.func_216348_a().setRemainingFireTicks(80);
	}
