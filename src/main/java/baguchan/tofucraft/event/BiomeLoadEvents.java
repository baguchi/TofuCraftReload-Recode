...
public class BiomeLoadEvents {
	...
	public static void loadingBiome(BiomeLoadingEvent event) {
		...
		if (event.getName().toString().contains("tofucraft:zunda_forest")) {
			generation.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, TofuWorldPlacements.BIG_ZUNDA_TOFU_MUSHROOM);
			generation.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, TofuWorldPlacements.PATCH_ZUNDA_TOFU_MUSHROOM);
		}
