...
public class FluidIngredient implements Predicate<FluidStack> {
	...
	private FluidIngredient(Value[] p_301101_) {
   ...
   */
	...
	public static FluidIngredient fromNetwork(FriendlyByteBuf p_43941_) {
		...
		return fromValues(Stream.generate(() -> new FluidValue(p_43941_.readFluidStack())).limit(size));
	}
	...
	private static Codec<FluidIngredient> codec(boolean p_298496_) {
		...
		return ExtraCodecs.either(codec, Value.CODEC).flatComapMap((p_296900_) -> {
			return p_296900_.map(FluidIngredient::new, (p_296903_) -> {
				return new FluidIngredient(new Value[]{p_296903_});
			});
		}, (p_296899_) -> {
	...
	public static record FluidValue(FluidStack fluidStack) implements Value {
		...
		static final Codec<FluidValue> CODEC = RecordCodecBuilder.create((p_300421_) -> {
			...
			})).apply(p_300421_, FluidValue::new);
		});

		public boolean equals(Object p_300135_) {
			if (!(p_300135_ instanceof FluidValue ingredient$itemvalue)) {
				return false;
			} else {
	...
	public static record TagValue(TagKey<Fluid> tag) implements Value {
	...
	public interface Value {
		Codec<Value> CODEC = ExtraCodecs.xor(FluidValue.CODEC, TagValue.CODEC).xmap((p_300070_) -> {
