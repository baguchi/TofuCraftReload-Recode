...
public class DefaultTofuFeatures {
	public static RandomPatchConfiguration NETHER_SOYBEAN_CLUSTER = (new RandomPatchConfiguration.GrassConfigurationBuilder(new SimpleStateProvider(TofuBlocks.SOYBEAN_SOUL.defaultBlockState().setValue(SoybeanSoulCropsBlock.AGE, 7)), SimpleBlockPlacer.INSTANCE)).tries(64).noProjection().build();
	...
	public static final ConfiguredFeature<?, ?> NETHER_SOYBEAN = register("tofucraft:nether_soybean", (Feature.RANDOM_PATCH.configured(NETHER_SOYBEAN_CLUSTER).range(Features.Decorators.FULL_RANGE)).rarity(1));
