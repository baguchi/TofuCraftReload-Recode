package baguchi.tofucraft.client.model.item;
...
public class TofuMetalBucketModel implements ItemModel {
	...
	private static final ModelDebugName DEBUG_NAME = () -> "TofuMetalBucketModel";
	...
	private TofuMetalBucketModel(Unbaked unbakedModel, BakingContext bakingContext, Matrix4fc transformation) {
	...
	public void update(ItemStackRenderState state, ItemStack stack, ItemModelResolver resolver, ItemDisplayContext context, @Nullable ClientLevel level, @Nullable ItemOwner owner, int seed) {
		...
		} else if (stack.get(TofuDataComponents.STORED_BLOCK) != null) {
			content = BuiltInRegistries.BLOCK.getKey(stack.get(TofuDataComponents.STORED_BLOCK).getBlock());
		}
		SimpleFluidContent fluid = null;
		if (content == null) {
			fluid = stack.getOrDefault(TofuDataComponents.STORED_FLUID, SimpleFluidContent.EMPTY);
	...
	public record Textures(Optional<Material> particle, Optional<Material> base, Optional<Material> fluid,
	                       Optional<Material> defaultContent) {
	...
	public record Unbaked(Textures textures, Fluid fluid, boolean flipGas,
	                      boolean applyFluidLuminosity) implements ItemModel.Unbaked {
		...
		public ItemModel bake(BakingContext context, Matrix4fc transformation) {
			return new TofuMetalBucketModel(this, context, transformation);
		}
