...
public class StickFoodItemRender implements IClientItemExtensions {
	...
	public boolean applyForgeHandTransform(PoseStack poseStack, PlayerRenderState playerRenderState, HumanoidArm arm, ItemStack itemInHand, float partialTick, float equipProcess, float swingProcess) {
		if (playerRenderState.avatarRenderState.isUsingItem && playerRenderState.avatarRenderState.getMainHandItemStack().equals(itemInHand)) {
			boolean flag2 = arm == HumanoidArm.RIGHT;
			int i = flag2 ? 1 : -1;
			float f = playerRenderState.avatarRenderState.ticksUsingItem;

			if (f < 0.8F) {
				float f2 = Mth.clamp(f + 0.2F, 0F, 1.0F);
				poseStack.translate(0.0F, 0.0F, (1.0F - f2) * this.scale);
			} else {
				poseStack.translate(0.0F, 0.0F, -0.2F * this.scale);
			}

			float f3 = 1.0F - (float) Math.pow(f, 27.0);
			poseStack.translate(0.1F, 0, 0);
			poseStack.translate(f3 * i * -0.3F + i * 0.4F + -0.15F, 0.0F, -0.3F - 0.15F * i);
			poseStack.rotateDegrees(Axis.YP, i * 45.0F);
			poseStack.rotateDegrees(Axis.YP, 45.0F + 180);
			poseStack.rotateDegrees(Axis.YP, i * 15.0F);
			poseStack.rotateDegrees(Axis.ZN, i * 90.0F);
			//this.applyItemArmTransform(poseStack, arm, swingProcess);

			return true;
		}

		return IClientItemExtensions.super.applyForgeHandTransform(poseStack, playerRenderState, arm, itemInHand, partialTick, equipProcess, swingProcess);
	}
	...
	private void applyEatTransform(PoseStack p_109331_, float p_109332_, HumanoidArm p_109333_, ItemStack p_109334_, Player p_346361_) {
		...
		p_109331_.rotateDegrees(Axis.YP, i * f3 * 90.0F);
		p_109331_.rotateDegrees(Axis.XP, f3 * 10.0F);
		p_109331_.rotateDegrees(Axis.ZP, i * f3 * 30.0F);
	}
