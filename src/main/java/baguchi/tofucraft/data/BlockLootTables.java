...
public class BlockLootTables extends BlockLootSubProvider {
	...
	protected void generate() {
		...
		add(TofuBlocks.BARREL_ADV_TOFUGEM.get(), applyExplosionDecay(TofuBlocks.BARREL_ADV_TOFUGEM.get(), LootTable.lootTable().withPool(LootPool.lootPool().add(LootItem.lootTableItem(TofuItems.ADVANCE_TOFUGEM.get())).when(tofugemAdvBuilder).apply(SetItemCountFunction.setCount(ConstantValue.exactly(3.0F))).add(LootItem.lootTableItem(Blocks.BARREL)).apply(SetItemCountFunction.setCount(ConstantValue.exactly(1))).add(LootItem.lootTableItem(TofuBlocks.BARREL_ADV_TOFUGEM)).when(InvertedLootItemCondition.invert(tofugemAdvBuilder)).when(ExplosionCondition.survivesExplosion()))));
