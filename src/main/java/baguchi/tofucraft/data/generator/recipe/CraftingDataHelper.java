...
public abstract class CraftingDataHelper extends RecipeProvider {
	public CraftingDataHelper(BootstrapContext<Recipe<?>> recipeOutput, BootstrapContext<Advancement> advancementOutput) {
		super(recipeOutput, advancementOutput);
	}

	public ShapedRecipeBuilder makeSign(Supplier<? extends SignBlock> signOut, Supplier<? extends Block> planksIn) {
		return ShapedRecipeBuilder.shaped(this.items, RecipeCategory.DECORATIONS, signOut.get(), 3)
				.pattern("PPP")
				.pattern("PPP")
				.pattern(" / ")
	...
	protected ShapedRecipeBuilder makeHangingSign(Supplier<? extends CeilingHangingSignBlock> result, Supplier<? extends Block> log) {
		return ShapedRecipeBuilder.shaped(this.items, RecipeCategory.DECORATIONS, result.get(), 6)
				.pattern("| |")
				.pattern("###")
				.pattern("###")
				.define('#', log.get())
	...
	protected final void foodCooking(Supplier<? extends ItemLike> material, Supplier<? extends ItemLike> result, float xp, RecipeOutput consumer) {
		SimpleCookingRecipeBuilder.smelting(Ingredient.of(material.get()), RecipeCategory.FOOD, CookingBookCategory.FOOD, result.get(), xp, 200).unlockedBy("has_item", has(material.get())).save(consumer, prefix("smelting_" + BuiltInRegistries.ITEM.getKey(result.get().asItem()).getPath()));
	...
	protected final void foodCooking(Supplier<? extends ItemLike> material, Supplier<? extends ItemLike> result, float xp, RecipeOutput consumer, String recipeName) {
		SimpleCookingRecipeBuilder.smelting(Ingredient.of(material.get()), RecipeCategory.FOOD, CookingBookCategory.FOOD, result.get(), xp, 200).unlockedBy("has_item", has(material.get())).save(consumer, prefix("smelting_" + recipeName));
	...
	public void tofuDiamondSmithing(RecipeOutput consumer, ItemLike smithItem, RecipeCategory recipeCategory, Supplier<? extends Item> result) {
		SmithingTransformRecipeBuilder.smithing(Ingredient.of(TofuItems.TOFU_UPGRADE_SMITHING_TEMPLATE.get()), Ingredient.of(smithItem), Ingredient.of(TofuBlocks.DIAMOND_TOFU.get()), recipeCategory, result.get()).unlocks("has_item", has(TofuBlocks.DIAMOND_TOFU.get())).save(consumer, prefix(BuiltInRegistries.ITEM.getKey(result.get().asItem()).getPath() + "_smithing"));
	}
	...
	protected final void helmetItem(RecipeOutput consumer, String name, Supplier<? extends ItemLike> result, Supplier<? extends ItemLike> material) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.COMBAT, result.get())
				.pattern("###")
				.pattern("# #")
				.define('#', material.get())
	...
	protected final void pickaxeItem(RecipeOutput consumer, String name, Supplier<? extends ItemLike> result, Supplier<? extends ItemLike> material, TagKey<Item> handle) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.TOOLS, result.get())
				.pattern("###")
				.pattern(" X ")
				.pattern(" X ")
				.define('#', material.get())
	...
	protected final void tofuBlockItem(RecipeOutput consumer, Supplier<? extends ItemLike> result, Supplier<? extends ItemLike> material) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, result.get())
				.pattern("##")
				.pattern("##")
				.define('#', material.get())
				.unlockedBy("has_item", has(material.get()))
	...
	protected final void ladderItem(RecipeOutput consumer, Supplier<? extends ItemLike> result, Supplier<? extends ItemLike> material) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, result.get(), 6)
				.pattern("# #")
				.pattern("###")
				.pattern("# #")
				.define('#', material.get())
	...
	protected final void buildingTofuBlockItem(RecipeOutput consumer, Supplier<? extends ItemLike> result, Supplier<? extends ItemLike> material) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, result.get(), 4)
				.pattern("##")
				.pattern("##")
				.define('#', material.get())
				.unlockedBy("has_item", has(material.get()))
	...
	protected final void buildingTofuChiseledItem(RecipeOutput consumer, Supplier<? extends ItemLike> result, Supplier<? extends ItemLike> material) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, result.get(), 1)
				.pattern("#")
				.pattern("#")
				.define('#', material.get())
				.unlockedBy("has_item", has(material.get()))
	...
	public void makeStairs(RecipeOutput consumer, Supplier<? extends ItemLike> stairsOut, Supplier<? extends ItemLike> blockIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, stairsOut.get(), 4)
				.pattern("M  ")
				.pattern("MM ")
				.pattern("MMM")
	...
	public void makeSlab(RecipeOutput consumer, Supplier<? extends ItemLike> slabOut, Supplier<? extends ItemLike> blockIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, slabOut.get(), 6)
				.pattern("MMM")
				.define('M', blockIn.get())
				.unlockedBy("has_item", has(blockIn.get())).save(consumer);
	}
	...
	public void makeSolidFence(RecipeOutput consumer, Supplier<? extends ItemLike> fenceOut, Supplier<? extends ItemLike> blockIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, fenceOut.get(), 6)
				.pattern("MMM")
				.pattern("MMM")
				.define('M', blockIn.get())
				.unlockedBy("has_item", has(blockIn.get())).save(consumer);
	...
	public void makeTorch(RecipeOutput consumer, Supplier<? extends ItemLike> torchOut, Supplier<? extends ItemLike> itemIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.DECORATIONS, torchOut.get(), 4)
				.pattern("C")
				.pattern("M")
	...
	public void makeDoor(RecipeOutput consumer, Supplier<? extends Block> doorOut, Supplier<? extends Block> plankIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.DECORATIONS, doorOut.get(), 3)
				.pattern("PP")
				.pattern("PP")
				.pattern("PP")
	...
	public void makeTrapdoor(RecipeOutput consumer, Supplier<? extends Block> trapdoorOut, Supplier<? extends ItemLike> plankIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.DECORATIONS, trapdoorOut.get(), 2)
				.pattern("PPP")
				.pattern("PPP")
				.define('P', plankIn.get())
				.unlockedBy("has_" + BuiltInRegistries.ITEM.getKey(plankIn.get().asItem()).getPath(), has(plankIn.get())).save(consumer);
	...
	public void makeMetalTrapdoor(RecipeOutput consumer, Supplier<? extends Block> trapdoorOut, Supplier<? extends ItemLike> plankIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.DECORATIONS, trapdoorOut.get(), 8)
				.pattern("PP")
				.pattern("PP")
				.define('P', plankIn.get())
				.unlockedBy("has_" + BuiltInRegistries.ITEM.getKey(plankIn.get().asItem()).getPath(), has(plankIn.get())).save(consumer);
	...
	public void makeWoodFence(RecipeOutput consumer, Block fenceOut, Block blockIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, fenceOut, 3)
				.pattern("MSM")
				.pattern("MSM")
				.define('M', blockIn)
	...
	public void makeFenceGate(RecipeOutput consumer, Block fenceOut, Block blockIn) {
		ShapedRecipeBuilder.shaped(this.items, RecipeCategory.BUILDING_BLOCKS, fenceOut)
				.pattern("SMS")
				.pattern("SMS")
				.define('M', blockIn)
				.define('S', Tags.Items.RODS_WOODEN)
