...
public class NetherFukumameEntity extends FukumameEntity {
	...
	public void handleEntityEvent(byte id) {
		if (id == 3) {
			double d0 = 0.08D;
			for (int i = 0; i < 6; i++) {
				this.level().addParticle(new ItemParticleOption(ParticleTypes.ITEM, TofuItems.SEEDS_SOYBEANS_NETHER.get()), getX(), getY(), getZ(), (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D);
			}
		}
		if (id == 4) {
	...
	protected void onHitEntity(EntityHitResult p_37404_) {
		...
		if (this.isOnFire()) {
			p_37404_.getEntity().igniteForSeconds(4);
		}
	}
}