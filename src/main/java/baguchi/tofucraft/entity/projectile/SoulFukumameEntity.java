...
public class SoulFukumameEntity extends FukumameEntity {
	...
	public void handleEntityEvent(byte id) {
		if (id == 3) {
			double d0 = 0.08D;
			for (int i = 0; i < 6; i++) {
				this.level().addParticle(new ItemParticleOption(ParticleTypes.ITEM, TofuItems.SEEDS_SOYBEANS_SOUL.get()), getX(), getY(), getZ(), (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D, (this.random.nextFloat() - 0.5D) * 0.08D);
			}
		}
		if (id == 4) {
