...
public class ModNetherFeatures {
	...
	public static void bootstrap(BootstrapContext<ConfiguredFeature<?, ?>> context) {
		FeatureUtils.register(context, NETHER_SOYBEAN, Feature.SIMPLE_BLOCK, new SimpleBlockConfiguration(BlockStateProvider.simple(TofuBlocks.SOYBEAN_NETHER.get().defaultBlockState().setValue(SoybeanNetherCropsBlock.AGE, 7))));
		FeatureUtils.register(context, SOUL_SOYBEAN, Feature.SIMPLE_BLOCK, new SimpleBlockConfiguration(BlockStateProvider.simple(TofuBlocks.SOYBEAN_SOUL.get().defaultBlockState().setValue(SoybeanSoulCropsBlock.AGE, 7))));
	}
